...
def print_system_info():
    ...
    print(
        f"GPU Memory:    {info.gpu_memory_total_gb:.1f} GB total, {info.gpu_memory_used_gb:.1f} GB used, {info.gpu_memory_free_gb:.1f} GB free"
    )
...
def verify_environment() -> Dict[str, Any]:
    ...
    if not results["ngc_container"]:
        results["issues"].append(
            "Not running in NGC container - PyTorch may not work correctly"
        )
